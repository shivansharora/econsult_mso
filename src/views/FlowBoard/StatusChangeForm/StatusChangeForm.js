...
const ChangeStatus = props => {
	...
	useEffect(() => {
		...
		const fetchProjects = () => {
			if (localStorage.getItem("jwt") != '' || localStorage.getItem("jwt") !== undefined) {
				...
				axios.get('/get_mso_flowboard_status', { headers: { Authorization: token } }).then(response => {
					if (mounted) {
	...
	function Aller() {
		...
		if (localStorage.getItem("jwt") != '' || localStorage.getItem("jwt") !== undefined) {
			let token = "Bearer " + localStorage.getItem("jwt");
			axios.put(`/update_flow_board_status/${props.match.params.id}/${props.match.params.type}`, { status: values.status }, { headers: { Authorization: token } }).then(response => {
